...
//!   * the target shell can host the resume (native Linux shell or WSL under Windows;
//!     native Windows PowerShell/cmd is OUT OF SCOPE for v1 — `claude --resume` is a
//!     Unix invocation).
//!
...
/// a native Linux shell, or WSL under Windows. Native Windows (PowerShell/cmd) is OUT
/// OF SCOPE — `claude --resume` is a Unix invocation, so the decision refuses to
/// inject it into a Windows-native shell rather than emit a broken line.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ResumeTarget {
    /// A native Linux shell, or a WSL shell under Windows — resume is supported here.
    UnixOrWsl,
    /// A native Windows shell (PowerShell / cmd) — resume is out of scope for v1.
    WindowsNative,
}
...
impl ResumeTarget {
    ...
    pub fn classify_shell(shell: &str) -> Self {
        ...
        match base {
            // Windows-native shells — out of scope for v1 resume.
    ...
    /// Can this target host the v1 resume?
    fn supports_resume(self) -> bool {
        matches!(self, ResumeTarget::UnixOrWsl)
    }
}
...
pub enum ResumeSkipReason {
    ...
    NoConversation,
    /// The execution target cannot host the resume (native Windows, out of scope v1).
    UnsupportedTarget,
...
mod tests {
    ...
    /// A native Windows shell target refuses the resume (out of scope v1) even with
    /// every other gate open.
    #[test]
    fn windows_native_target_is_unsupported() {
        let claude = ClaudeCodeAdapter;
        let mut inputs = resumable("sid-1");
        inputs.target = ResumeTarget::WindowsNative;
        assert_eq!(
            decide_resume(&inputs, &claude),
            ResumeDecision::Skip(ResumeSkipReason::UnsupportedTarget)
        );
    }
    ...
    fn close_warning_message_distinguishes_agents_and_names_terminal() {
        ...
        assert!(m2.contains("terminal term-xyz"), "falls back to the id: {m2}");
        assert!(!m2.contains("workspace"), "no workspace clause when none: {m2}");
    }
    ...
    /// Shell classification: WSL launchers + Unix shells are resumable; the three
    /// Windows-native shells are not (with or without `.exe`, bare or path).
    #[test]
    fn classify_shell_routes_targets() {
        ...
        for win in [
            ...
            assert_eq!(
                ...
                "{win} should be a Windows-native (unsupported) target"
            );
        }
    }
}
