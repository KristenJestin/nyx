...
impl<R: CommandRunner> PortlessManager<R> {
    ...
    fn run_checked(&self, args: &[&str]) -> Result<CommandOutcome, PortlessError> {
        ...
        match self.runner.run(args) {
            ...
            Err(e) => Err(PortlessError::CommandFailed { command, message: e.to_string() }),
        }
    }
}
...
impl SystemRunner {
    ...
    fn base() -> std::process::Command {
        let mut cmd = std::process::Command::new("portless");
...
impl CommandRunner for SystemRunner {
    ...
    fn run(&self, args: &[&str]) -> std::io::Result<CommandOutcome> {
        ...
        Ok(CommandOutcome { success: out.status.success(), message })
    }
}
...
pub mod fake {
    ...
    impl FakeRunner {
        ...
        pub fn available() -> Self {
            Self {
                available: true,
                outcome: Some(CommandOutcome { success: true, message: String::new() }),
        ...
        pub fn missing() -> Self {
            Self { available: false, outcome: None, calls: Mutex::new(Vec::new()) }
        }
        ...
        pub fn failing(message: &str) -> Self {
            Self {
                available: true,
                outcome: Some(CommandOutcome { success: false, message: message.to_string() }),
                calls: Mutex::new(Vec::new()),
            }
        }
...
mod tests {
    ...
    fn enable_surfaces_url_and_runs_exact_argv() {
        ...
        let url = {
            ...
            m.enable(8765).expect("enable succeeds with portless present")
        };
        ...
        assert_eq!(owned.calls.lock().unwrap().len(), 1, "exactly one portless call");
    }
    ...
    fn enable_when_portless_missing_is_clear_error_and_no_spawn() {
        ...
        assert_eq!(owned.calls.lock().unwrap().len(), 0, "no command spawned when absent");
    }

    #[test]
    fn disable_when_portless_missing_is_clear_error() {
        let mgr = PortlessManager::new(fake::FakeRunner::missing());
        let err = mgr.disable().expect_err("missing portless must error on disable too");
        assert_eq!(err, PortlessError::NotInstalled);
    }
