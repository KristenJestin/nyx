...
pub fn classify(shell: &str) -> Shell {
    ...
    let base = shell
        ...
        .unwrap_or(shell)
        .trim_start_matches('-');
    let stem = base.strip_suffix(".exe").unwrap_or(base).to_ascii_lowercase();
    match stem.as_str() {
        "bash" => Shell::Bash,
        "zsh" => Shell::Zsh,
...
mod tests {
    ...
    fn classifies_unsupported_shells_as_unsupported() {
        ...
        assert_eq!(classify("C:\\Windows\\System32\\cmd.exe"), Shell::Unsupported);
    ...
    fn bash_snippet_sources_user_rc_and_emits_c_and_d() {
        let s = bash_snippet();
        assert!(s.contains(". \"$HOME/.bashrc\""), "sources user bashrc first");
        assert!(s.contains(r"\033]133;C\007"), "emits pre-exec 133;C");
        assert!(s.contains(r"\033]133;D;%s\007"), "emits command-end 133;D with code");
        assert!(s.contains("PROMPT_COMMAND="), "registers precmd via PROMPT_COMMAND");
        assert!(s.contains("trap '__nyx_osc133_preexec' DEBUG"), "preexec via DEBUG trap");
    }

    #[test]
    fn zsh_snippet_sources_user_rc_and_registers_hooks() {
        let s = zsh_snippet();
        assert!(s.contains("source \"$__nyx_real_zdotdir/.zshrc\""), "sources real zshrc");
        assert!(s.contains(r"\033]133;C\007"), "preexec 133;C");
        assert!(s.contains(r"\033]133;D;%s\007"), "precmd 133;D with $? code");
    ...
    fn powershell_snippet_wraps_prompt_and_recovers_exit_code() {
        ...
        assert!(s.contains("__nyxOriginalPrompt"), "preserves the original prompt");
        assert!(s.contains("if ($?) { 0 } elseif ($null -ne $LASTEXITCODE)"), "recovers exit code");
    ...
    fn build_zsh_plan_sets_zdotdir_to_a_real_dir_with_zshrc() {
        ...
        assert!(
            plan.env.iter().any(|(k, _)| k == &OsString::from("NYX_REAL_ZDOTDIR")),
