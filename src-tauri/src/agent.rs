...
pub trait AgentAdapter: Send + Sync {
    ...
    fn install_integration(
        ...
        let (Some(descriptor), Some(cli)) = (self.plugin_install(resource_dir, app_data_dir), self.plugin_cli())
        else {
    ...
    fn parse_event(&self, payload: &serde_json::Value) -> Option<AgentEvent> {
        ...
        match event_name {
            "SessionStart" => {
                ...
                let metadata_json = payload.get("source").and_then(|v| v.as_str()).map(|src| {
    ...
    fn known_risks(&self) -> Vec<KnownRisk> {
        vec![
            ...
            KnownRisk {
                ...
                detail: "Exact resume targets a native Linux shell or WSL under Windows; native \
                         Windows (PowerShell/cmd) resume is out of scope for v1.",
            },
        ]
    }
}
...
impl ClaudePluginCli {
    /// Run `claude <args>` and capture status + output. Maps a missing binary
    /// (`NotFound`) to [`crate::plugin::PluginError::CliNotFound`]. `tolerate_failure`
    /// returns `Ok` even on a non-zero exit (for best-effort remove/uninstall, where
    /// "already absent" is reported as a non-fatal message), capturing nothing.
    fn run(&self, args: &[&str], tolerate_failure: bool) -> Result<std::process::Output, crate::plugin::PluginError> {
        let output = std::process::Command::new(&self.bin)
            ...
            .output();
        match output {
            Ok(out) if out.status.success() || tolerate_failure => Ok(out),
            Ok(out) => {
                let mut msg = String::from_utf8_lossy(&out.stderr).into_owned();
                if msg.trim().is_empty() {
                    msg = String::from_utf8_lossy(&out.stdout).into_owned();
                }
                Err(crate::plugin::PluginError::CliFailed { command: args.join(" "), output: msg })
            }
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                Err(crate::plugin::PluginError::CliNotFound)
            }
            Err(e) => Err(crate::plugin::PluginError::Io(e)),
        }
    }
}
...
impl crate::plugin::PluginCli for ClaudePluginCli {
    fn marketplace_add(&self, dir: &std::path::Path) -> Result<(), crate::plugin::PluginError> {
        ...
        self.run(&["plugin", "marketplace", "add", &dir, "--scope", "user"], false)?;
        Ok(())
    }
    ...
    fn uninstall(&self, install_id: &str) -> Result<(), crate::plugin::PluginError> {
        ...
        self.run(&["plugin", "uninstall", install_id, "--scope", "user", "-y"], true)?;
        Ok(())
    }

    fn marketplace_remove(&self, marketplace: &str) -> Result<(), crate::plugin::PluginError> {
        // Best-effort: "not found" is acceptable.
        self.run(&["plugin", "marketplace", "remove", marketplace, "--scope", "user"], true)?;
        Ok(())
    }
    ...
    fn marketplace_list(&self) -> Result<Vec<crate::plugin::MarketplaceEntry>, crate::plugin::PluginError> {
        ...
        let parsed: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap_or(serde_json::Value::Null);
        let entries = parsed
            .as_array()
            .map(|arr| {
                arr.iter()
...
mod tests {
    ...
    fn all_four_agent_kinds_are_representable() {
        ...
        for kind in [
            ...
            assert!(adapter.is_some(), "{kind} must be representable in the registry");
            assert_eq!(adapter.unwrap().kind(), kind, "registry keyed by agent_kind");
        }
    ...
    fn per_adapter_limits_are_exposable() {
        ...
        assert!(claude.supports_exact_resume(), "claude supports exact resume");
        assert!(claude.supports_end_event(), "claude emits an end event (consumed)");
        ...
        for kind in [db::AGENT_KIND_CODEX, db::AGENT_KIND_OPENCODE, db::AGENT_KIND_CUSTOM] {
            let a = reg.get(kind).unwrap();
            assert!(!a.supports_exact_resume(), "{kind} has no production resume in v1");
    ...
    fn claude_builds_exact_resume_command() {
        ...
        assert_eq!(claude.build_resume_command("").as_deref(), None, "empty id → no command");
        ...
        assert_eq!(codex.build_resume_command("abc").as_deref(), None, "placeholder builds nothing");
    }
    ...
    fn claude_parses_session_start() {
        ...
        let ev = claude.parse_event(&payload).expect("recognized SessionStart");
        match ev {
            AgentEvent::Start(s) => {
    ...
    fn detect_routes_claude_invocations() {
        let reg = AgentRegistry::default();
        for cmd in ["claude", "claude --resume x", "/usr/bin/claude", "C:\\bin\\claude.exe -p"] {
            let a = reg.detect(cmd);
            assert!(a.is_some(), "{cmd} should detect an adapter");
            assert_eq!(a.unwrap().kind(), db::AGENT_KIND_CLAUDE_CODE, "{cmd} → claude_code");
        }
        assert!(reg.detect("vim file.txt").is_none(), "non-claude detects nothing");
    ...
    fn claude_plugin_install_targets_a_stable_dir() {
        ...
        let _guard = crate::CLAUDE_ENV_TEST_LOCK.lock().unwrap_or_else(|p| p.into_inner());
        ...
        std::fs::write(plugin_dir.join(".claude-plugin").join("marketplace.json"), "{}").unwrap();
        ...
        let desc = adapter.plugin_install(None, Some(&app_data)).expect("descriptor resolves");
        ...
        assert_ne!(desc.install_dir, desc.source_dir, "registered path is stable, not the source");
    ...
    fn claude_install_integration_fails_when_cli_absent() {
        ...
        match outcome {
            InstallOutcome::Failed(msg) => assert!(msg.contains("claude"), "absent CLI → actionable error: {msg}"),
