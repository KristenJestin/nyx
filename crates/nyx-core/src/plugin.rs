...
pub enum PluginError {
    ...
    Io(std::io::Error),
    /// The CLI subcommand did not exit within its wall-clock budget and was killed
    /// (PRD-5 task #4 hardening: `claude` figé ne fige pas l'app). Carries the
    /// subcommand + the elapsed seconds so the UI / logs can report the stall. A hung
    /// `claude` (e.g. a network call with no timeout) thus surfaces as a typed,
    /// bounded error rather than freezing the caller forever (the `.output()` trap).
    Timeout { command: String, after_secs: u64 },
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            PluginError::Timeout {
                ...
                f,
                "`claude {command}` did not finish within {after_secs}s and was terminated — \
                 the Claude CLI appears to be stuck"
            ),
        }
    }
}
...
pub fn install_with(
    ...
    cli: &dyn PluginCli,
) -> Result<PluginChange, PluginError> {
    let entries = cli.marketplace_list()?;
    let already_current = is_registered_at(&entries, &install.marketplace, &install.install_dir);
...
mod tests {
    ...
    fn write_bundled(dir: &Path) -> PathBuf {
        ...
        std::fs::write(
            plugin.join(".claude-plugin").join("marketplace.json"),
            r#"{"name":"nyx"}"#,
        )
        .unwrap();
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"nyx":{{"type":"http","url":"http://127.0.0.1:{MCP_PORT_PLACEHOLDER}/mcp"}}}}"#
            ),
        )
        .unwrap();
        plugin
    ...
    impl PluginCli for FakeCli {
        ...
        fn install(&self, install_id: &str) -> Result<(), PluginError> {
            ...
            self.calls
                .borrow_mut()
                .push(format!("install {install_id}"));
        ...
        fn uninstall(&self, install_id: &str) -> Result<(), PluginError> {
            self.calls
                .borrow_mut()
                .push(format!("uninstall {install_id}"));
        ...
        fn marketplace_remove(&self, marketplace: &str) -> Result<(), PluginError> {
            self.calls
                .borrow_mut()
                .push(format!("remove {marketplace}"));
            ...
            Ok(self
                ...
                .map(|p| {
                    vec![MarketplaceEntry {
                        name: CLAUDE_MARKETPLACE.to_string(),
                        path: Some(p),
                    }]
                })
                .unwrap_or_default())
    ...
    fn install_copies_to_stable_dir_and_drives_cli() {
        ...
        assert!(
            stable
                ...
                .exists(),
            "bundled content copied to stable dir"
        );
        let calls = cli.calls.borrow().clone();
        assert!(
            calls
                .iter()
                .any(|c| c == &format!("add {}", stable.display())),
            "registered the STABLE path, not the source: {calls:?}"
        );
        assert!(calls
            .iter()
            .any(|c| c == "install nyx-claude-integration@nyx"));
    ...
    fn install_reports_updated_when_bundled_content_changes() {
        ...
        assert!(
            copied.contains("SessionStart"),
            "stable copy refreshed with new content"
        );
    }
    ...
    fn install_templates_mcp_port_into_copied_descriptor() {
        ...
        assert!(
            copied.contains("127.0.0.1:9931/mcp"),
            "live port templated in: {copied}"
        );
        assert!(
            !copied.contains(MCP_PORT_PLACEHOLDER),
            "placeholder substituted out"
        );
        ...
        assert!(
            src.contains(MCP_PORT_PLACEHOLDER),
            "bundled source stays port-agnostic"
        );
    }
    ...
    fn install_re_templates_descriptor_on_port_change() {
        ...
        let copied = std::fs::read_to_string(stable.join(MCP_DESCRIPTOR_FILE)).unwrap();
        assert!(
            copied.contains("127.0.0.1:7000/mcp"),
            "descriptor re-templated to new port"
        );
        let calls = cli.calls.borrow().clone();
        assert!(
    ...
    fn template_mcp_port_is_pure_and_robust() {
        let out = template_mcp_port(
            format!("a{MCP_PORT_PLACEHOLDER}b{MCP_PORT_PLACEHOLDER}").as_bytes(),
            42,
        );
    ...
    fn install_propagates_version_bump_through_caches() {
        ...
        assert!(
            !cli.calls
                .borrow()
                .iter()
                .any(|c| c.starts_with("mkt-update")),
    ...
    fn registered_path_is_stable_across_source_dir_move() {
        ...
        assert_eq!(
            cli.registered.borrow().clone(),
            registered_after_dev,
            "registered path unchanged across a source move"
        );
        assert_eq!(cli.registered.borrow().as_deref(), Some(stable.as_path()));
    }
    ...
    fn install_self_heals_a_dead_path_entry() {
        ...
        *cli.registered.borrow_mut() = Some(dir.join("DEAD-stale-path"));
        ...
        assert!(matches!(
            change,
            PluginChange::Added | PluginChange::Updated
        ));
        assert_eq!(
            cli.registered.borrow().as_deref(),
            Some(stable.as_path()),
            "dead path healed to the stable path"
        );
        assert!(stable
            .join(".claude-plugin")
            .join("marketplace.json")
    ...
    fn reconcile_absent_is_noop() {
        ...
        assert_eq!(
            reconcile_with(&inst, &cli).unwrap(),
            ReconcileOutcome::SkippedAbsent
        );
        assert!(
            cli.calls.borrow().is_empty(),
            "absent → no CLI writes (never install on boot)"
        );
    ...
    fn reconcile_present_unchanged_is_noop() {
        ...
        assert_eq!(
            cli.calls.borrow().len(),
            calls_before,
            "no re-register when already current"
        );
    }
    ...
    fn remove_drives_cli_and_strips_legacy_settings() {
        ...
        assert!(
            remove_with(&inst, &cli).unwrap(),
            "legacy keys were stripped"
        );
        ...
        assert!(
            v["extraKnownMarketplaces"].get("nyx").is_none(),
            "legacy nyx marketplace stripped"
        );
        assert!(
            v["enabledPlugins"]
                .get("nyx-claude-integration@nyx")
                .is_none(),
            "legacy nyx enabledPlugins stripped"
        );
        assert_eq!(
            v["extraKnownMarketplaces"]["claude-code-warp"]["source"]["repo"],
    ...
    fn install_surfaces_cli_not_found() {
        ...
        assert!(
            matches!(err, PluginError::CliNotFound),
            "claude absent → typed CliNotFound"
        );
        // The display message is user-actionable (surfaced in the UI).
        assert!(
            err.to_string().contains("claude"),
            "error mentions the missing CLI"
        );
    }
    ...
    /// GUARD (finding #51): the four files that make a loadable plugin must all exist in
    /// nyx-core's own `resources/claude-plugin/` source tree, or any shell that bundles
    /// them (the Tauri adapter's `tauri.conf.json`, a future Electron core-host) would
    /// ship an incomplete plugin. The original bug: `.mcp.json` was missing, so
    /// `plugin.json`'s `"mcpServers": "./.mcp.json"` pointed at nothing and Claude loaded
    /// the plugin with `MCP servers (0)`. This crate owns the asset; the per-shell BUNDLE
    /// manifest is validated by each shell (the Tauri `tauri.conf.json` list is checked in
    /// the `nyx` adapter crate, which is where that config lives).
    #[test]
    fn nyx_core_ships_every_plugin_file() {
        ...
        for f in required {
            assert!(
                ...
                "nyx-core is missing the bundled plugin file `{f}` — every shell that \
                 bundles the plugin would ship it incomplete"
            );
        }
    }
    ...
    fn plugin_enabled_when_enabledplugins_true() {
        ...
        .unwrap();
        assert!(plugin_enabled_in_settings(
            &settings,
            &claude_plugin_install_id()
        ));
    }
    ...
    fn plugin_not_enabled_when_value_false() {
        ...
        std::fs::write(
            &settings,
            r#"{"enabledPlugins":{"nyx-claude-integration@nyx":false}}"#,
        )
        .unwrap();
        assert!(!plugin_enabled_in_settings(
            &settings,
    ...
    fn claude_plugin_enabled_honors_the_settings_seam() {
        ...
        std::fs::write(
            &settings,
            r#"{"enabledPlugins":{"nyx-claude-integration@nyx":true}}"#,
        )
        .unwrap();
        assert!(
            claude_plugin_enabled(),
            "enabledPlugins true → enabled via the seam"
        );
        ...
        assert!(
            !claude_plugin_enabled(),
            "enabledPlugins entry gone → not enabled via the seam"
        );
