...
//!
//! - `C` (pre-exec) → marks that a REAL command is now running. (We deliberately
//!   key this off `C`, not `B`: `B` only means the prompt finished and input may
//!   begin; a bare Enter at an empty prompt emits `B` then `D` with no `C`, and
//!   must NOT flash running.)
...
//!   state machine, never left as a stale `running`) — **but ONLY if a `C`
//!   (pre-exec) was observed since the last settle.** A `D` with NO preceding `C`
//!   (the shell's FIRST prompt at spawn emits `D;0` because `$?` is true, BEFORE
//!   any command and with no `C`; likewise a bare Enter on an empty prompt) is a
//!   PHANTOM end and is IGNORED — the terminal stays `idle`, no badge. This is the
//!   exact symmetric counterpart of "running is keyed on `C`, not `B`": a settle
//!   needs provenance (a command actually ran), or it would paint a green success
//!   dot on a freshly-spawned terminal where nothing has run. See
//!   [`ExecStateMachine`].
...
/// A SETTLED command result the exec-state machine asks its caller to record: the
/// terminal-agnostic outcome of a `D` (command-end) that was preceded by a real
/// `C` (pre-exec). The caller (Tauri bridge / nyx-napi pump) maps this onto its
/// persisted exec-state (`success`/`error` + exit code) and the badge.
#[cfg_attr(not(test), allow(dead_code))]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ExecOutcome {
    /// `D;0` after a real command → `success`.
    Success,
    /// `D;<non-zero>` or `D` with a missing/garbage code, after a real command →
    /// `error`. `exit_code` is `Some(code)` when the `D` carried a parseable code,
    /// `None` for a finished-but-unknown result.
    Error { exit_code: Option<i32> },
}

/// The OSC 133 → exec-state STATE MACHINE, with the PROVENANCE GUARD that makes a
/// `D` (command-end) settle a result ONLY when a real command actually ran.
///
/// # Why this exists (the phantom-success-dot fix)
///
/// A naive mapping settles EVERY `CommandEnd` to `success`/`error`. That is wrong
/// at shell spawn: the shell's FIRST prompt emits `133;D;0` (because `$?` is true
/// on a fresh shell) BEFORE any command and with NO preceding `133;C` (see the
/// PowerShell capture in this module's docs:
/// `…]133;D;0 BEL ]133;A BEL PS …> ]133;B BEL`). A bare Enter on an empty prompt
/// does the same. Mapped naively, that phantom `D` paints a GREEN success dot
/// (and an unread notification) on a terminal where nothing has run.
///
/// The guard: a `CommandEnd` settles an [`ExecOutcome`] ONLY if a [`PreExec`](Osc133Event::PreExec)
/// (`C`) has been observed SINCE THE LAST SETTLE. `C` is the marker that a real
/// command started; without it, the `D` is a prompt-initial / empty-Enter phantom
/// and is IGNORED (the terminal stays `idle`, no badge). This is the exact
/// symmetric counterpart of the existing "running is keyed on `C`, not `B`"
/// decision. The flag is cleared on each settle so the NEXT command needs its own
/// `C` (back-to-back `D`s without intervening `C` cannot double-settle).
///
/// `A`/`B`/`C` themselves never settle anything (they only arm/disarm the guard);
/// the OS busy signal — not OSC 133 — owns the `running` state, so this machine
/// emits ONLY settled `success`/`error` outcomes.
#[cfg_attr(not(test), allow(dead_code))]
#[derive(Debug, Default, Clone)]
pub struct ExecStateMachine {
    /// Whether a `133;C` (pre-exec — a real command started) has been seen since
    /// the last settle. A `D` only settles a result when this is true.
    saw_pre_exec: bool,
}

impl ExecStateMachine {
    /// A fresh machine for a newly-spawned terminal: no command has run yet, so a
    /// `D` before any `C` will be ignored.
    #[cfg_attr(not(test), allow(dead_code))]
    pub fn new() -> Self {
        Self::default()
    }

    /// Feed ONE decoded OSC 133 event. Returns `Some(outcome)` ONLY when this event
    /// is a `CommandEnd` (`D`) that a real `PreExec` (`C`) preceded since the last
    /// settle — the caller then persists/emits that settled result. Returns `None`
    /// for `A`/`B`/`C` (inert here) AND for a `D` with NO preceding `C` (a
    /// prompt-initial or empty-Enter PHANTOM — ignored, terminal stays `idle`).
    #[cfg_attr(not(test), allow(dead_code))]
    pub fn on_event(&mut self, event: Osc133Event) -> Option<ExecOutcome> {
        match event {
            // A real command started: arm the guard so the matching `D` may settle.
            Osc133Event::PreExec => {
                self.saw_pre_exec = true;
                None
            }
            // Prompt start / command start carry no exec-state meaning and do NOT
            // arm the guard (a bare Enter emits `B` then `D` with no `C`).
            Osc133Event::PromptStart | Osc133Event::CommandStart => None,
            // Command end: settle ONLY if a real command ran (a `C` was seen). The
            // shell's first prompt at spawn — and a bare Enter — emit a `D` with no
            // preceding `C`; that is a phantom end we IGNORE (no badge).
            Osc133Event::CommandEnd { exit_code } => {
                if !self.saw_pre_exec {
                    // Phantom `D` (prompt-initial / empty Enter): ignore, stay idle.
                    return None;
                }
                // Consume the arm so the NEXT command needs its own `C`.
                self.saw_pre_exec = false;
                Some(match exit_code {
                    Some(0) => ExecOutcome::Success,
                    Some(code) => ExecOutcome::Error {
                        exit_code: Some(code),
                    },
                    None => ExecOutcome::Error { exit_code: None },
                })
            }
        }
    }
...
mod tests {
    ...
    // --- Exec-state machine: the provenance guard (phantom-success-dot fix) ----

    /// Drive a whole byte stream through the machine and collect every settled
    /// outcome (the shape the bridge/pump produce). Mirrors how the real consumers
    /// loop `extract_events` → `machine.on_event`.
    fn settles(stream: &[u8]) -> Vec<ExecOutcome> {
        let mut sm = ExecStateMachine::new();
        extract_events(stream)
            .into_iter()
            .filter_map(|ev| sm.on_event(ev))
            .collect()
    }

    #[test]
    fn prompt_initial_d_without_c_does_not_settle() {
        // THE BUG: the shell's first prompt at spawn emits `A B D;0` (D;0 because
        // `$?` is true on a fresh shell) with NO preceding `C`. That phantom `D`
        // must NOT settle a success — the terminal stays idle, no green dot.
        let first_prompt = b"\x1b]133;A\x07PS C:\\> \x1b]133;B\x07\x1b]133;D;0\x07";
        assert_eq!(
            settles(first_prompt),
            vec![],
            "a D;0 with no preceding C (prompt-initial) must NOT settle a success"
        );
        // The exact PowerShell capture order from the module docs (D;0 then A then
        // B for the NEXT prompt) is also a phantom — still no settle.
        let ps_capture = b"\x1b]133;D;0\x07\x1b]133;A\x07PS D:\\> \x1b]133;B\x07";
        assert_eq!(
            settles(ps_capture),
            vec![],
            "the PowerShell spawn capture must not settle"
        );
    }

    #[test]
    fn bare_enter_on_empty_prompt_does_not_settle() {
        // A bare Enter at an empty prompt emits `B` then `D` with no `C` — same
        // phantom shape, must not flash a result.
        assert_eq!(
            settles(b"\x1b]133;B\x07\x1b]133;D;0\x07"),
            vec![],
            "B then D with no C (empty Enter) must not settle"
        );
    }

    #[test]
    fn real_command_c_then_d0_settles_success() {
        // C (a real command ran) → output → D;0 → success, exactly as before the
        // guard. Non-regression.
        let stream = b"\x1b]133;C\x07hello\r\n\x1b]133;D;0\x07";
        assert_eq!(settles(stream), vec![ExecOutcome::Success]);
    }

    #[test]
    fn real_command_c_then_d3_settles_error() {
        let stream = b"\x1b]133;C\x07boom\r\n\x1b]133;D;3\x07";
        assert_eq!(
            settles(stream),
            vec![ExecOutcome::Error { exit_code: Some(3) }]
        );
    }

    #[test]
    fn d_without_code_after_c_settles_error_unknown() {
        // A `D` with no parseable code, but after a real `C`, still settles (to an
        // error with unknown code) — never left as a stale running.
        assert_eq!(
            settles(b"\x1b]133;C\x07\x1b]133;D\x07"),
            vec![ExecOutcome::Error { exit_code: None }]
        );
    }

    #[test]
    fn guard_rearms_per_command_no_double_settle() {
        // First prompt phantom (ignored), then a REAL command (C D;0 → success),
        // then ANOTHER phantom D at the next idle prompt (no C since the settle →
        // ignored), then a second real command (C D;1 → error). Only the two real
        // commands settle, in order.
        let stream = concat_bytes(&[
            b"\x1b]133;A\x07PS> \x1b]133;B\x07\x1b]133;D;0\x07", // spawn prompt: phantom
            b"\x1b]133;C\x07ok\r\n\x1b]133;D;0\x07",             // real: success
            b"\x1b]133;A\x07PS> \x1b]133;B\x07\x1b]133;D;0\x07", // idle prompt: phantom again
            b"\x1b]133;C\x07bad\r\n\x1b]133;D;1\x07",            // real: error
        ]);
        assert_eq!(
            settles(&stream),
            vec![
                ExecOutcome::Success,
                ExecOutcome::Error { exit_code: Some(1) },
            ],
            "only the two real C-preceded commands settle; both phantom Ds are ignored"
        );
    }

    #[test]
    fn second_d_without_new_c_is_ignored() {
        // C D;0 settles success, then a bare D arrives with no new C: the guard was
        // consumed by the first settle, so the second D is a phantom and ignored.
        assert_eq!(
            settles(b"\x1b]133;C\x07\x1b]133;D;0\x07\x1b]133;D;5\x07"),
            vec![ExecOutcome::Success],
            "back-to-back Ds without an intervening C cannot double-settle"
        );
    }

    /// Concatenate byte slices (test helper for building multi-prompt streams).
