...
/// The result of an auto-attach pass over a terminal RECORD (parity with the Tauri
/// `AutoAttachResult`): the workspace the terminal is bound to after the pass (`None`
/// = loose) and whether the binding actually CHANGED (the front only reflects a
/// change so it never silently un-pins a manual terminal).
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Run the DB-backed auto-attach for a terminal RECORD given its live `cwd` — the
/// shell-agnostic body the Tauri `auto_attach_terminal` command and the Electron
/// core-host now SHARE. Reads the record's current binding + the known workspaces,
/// applies [`decide_attachment`] (hybrid auto/manual rule), and persists the decided
/// `attach_terminal(.., auto)` when it changed. `cwd` is already the live reading the
/// caller pulled (Linux `/proc`, OSC 7 elsewhere); `None`/empty ⇒ no reliable source
/// ⇒ no change. An unknown terminal id is a no-op (`{ None, false }`).
pub fn auto_attach_terminal(
    conn: &mut diesel::SqliteConnection,
    terminal_id: &str,
    cwd: Option<&str>,
) -> diesel::QueryResult<AutoAttachResult> {
    use crate::db;
    // Normalize the incoming cwd uniformly (the single canonical form the resolver
    // consumes regardless of source); an empty result is "no reliable cwd".
    let normalized = cwd.map(pathnorm::normalize).filter(|c| !c.is_empty());

    // Current binding of the terminal record. Unknown id ⇒ no-op (unattached).
    let Some(term) = db::get_terminal(conn, terminal_id)? else {
    ...
    // The candidate set is ONLY the already-known workspaces (auto-attach creates none).
