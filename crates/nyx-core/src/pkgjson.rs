...
fn normalize_glob(g: &str) -> String {
    g.replace('\\', "/").trim_matches('/').to_string()
}
...
pub fn discover_scripts(workspace_path: &str) -> DiscoveryResult {
    ...
    let root_json: Option<serde_json::Value> =
        std::fs::read_to_string(canon_ws.join("package.json"))
            .ok()
...
fn collect_package_files(dir: &Path, rel: &str, depth: usize, ctx: &mut DiscoveryCtx) {
    ...
    if candidate {
        ...
        if pkg.is_file() {
            if let Some(file) = read_package_file(&pkg, dir, ctx.canon_ws_norm, ctx.workspace_norm)
            {
                ctx.files.push(file);
            }
        }
...
// --- Shared command-source helpers (one implementation for every shell) ------
//
// The provenance/source logic the managed-command template CRUD needs — `infer`
// (categorize a manual command whose line is a PM invocation), `detach` (drop the
// package.json link when an edit drifts the command), and `read_script_body`
// (re-read a source script at call time). Lifted INTO nyx-core so the Tauri shell
// (`bridge.rs`), the Tauri MCP tools (`mcp_tools.rs`), AND the Electron core-host
// napi dispatcher (`nyx-napi`) all drive ONE copy — the long-tail surface the
// migration kept re-discovering is now wired from a single source of truth.
...
fn infer_package_manager_from_command(command: &str) -> Option<&'static str> {
    ...
    PackageManager::from_name(first).map(PackageManager::as_db_str)
}
...
/// `source_kind` the schema's CHECK allows), so an inferred command reads
/// consistently with a detected one. A command whose first token is not a known PM
/// is left untouched (both stay `None`).
///
/// The ONE implementation behind the UI `command_create` (both shells) and the MCP
/// `add_command` tool — no surface re-derives this rule.
pub fn infer_command_source(
    command: &str,
...
/// Whether replacing a sourced template's command with `new_command` should DETACH
/// its package.json source. It detaches when the new command is neither the detected
/// package-manager runner invocation for the source script (`pnpm dev`, …) NOR the
/// current raw script snapshot — i.e. the user edited the command away from the
/// canonical call so it no longer tracks the script. Callers only invoke this for an
/// actually-sourced template.
///
/// The ONE implementation behind the UI `command_update` (both shells) and the MCP
/// `update_command` tool — both apply the IDENTICAL detach rule.
pub fn command_detaches_source(template: &ManagedCommand, new_command: &str) -> bool {
...
/// Used by `source_refresh` / `resync` to re-read the source AT CALL TIME (never the
/// snapshot). Distinguishes a missing file from a missing script.
pub fn read_script_body(pkg_path: &str, script_name: &str) -> ScriptLookup {
...
/// `snapshot`. `command` is deliberately ABSENT — refresh never rewrites it. Mirrors
/// the Tauri `SourceRefreshResult` so both shells return one shape.
...
/// Re-read a template's source `package.json` and update ONLY the snapshot + status
/// (never `command`). The shared core behind the UI `command_source_refresh` on both
/// shells. Errors if the template is unknown or carries no package.json link.
pub fn source_refresh(
    conn: &mut SqliteConnection,
    id: &str,
) -> Result<SourceRefreshResult, String> {
    let template = db::get_template(conn, id)
    ...
    match read_script_body(&pkg_path, &script_name) {
        ...
        ScriptLookup::Body(body) => {
            ...
            db::set_template_source(conn, id, source).map_err(|e| e.to_string())?;
...
/// Re-read the template's source script's CURRENT raw body (at call time, not the
/// snapshot), REWRITE `command` to it, KEEP the source link, and refresh the snapshot
/// (now fresh). The shared core behind the UI `command_resync_source` on both shells.
/// Returns the adopted raw body. Errors if the template is unknown / unsourced, or the
/// file/script no longer exists. (The runtime guard "refused while running" is applied
/// by the caller, which owns the live runner.)
pub fn source_resync(conn: &mut SqliteConnection, id: &str) -> Result<String, String> {
    let template = db::get_template(conn, id)
        .map_err(|e| e.to_string())?
        .ok_or_else(|| format!("unknown command {id}"))?;
    let pkg_path = template
...
mod tests {
    ...
    fn node_modules_is_excluded_from_discovery() {
        ...
        assert!(
            find(&scripts, "start").is_some(),
            "the real api package is found"
        );
    }
    ...
    fn gitignored_nested_git_repo_is_still_discovered_repo_of_repos() {
        ...
        assert!(
            result
                .scripts
                .iter()
                .all(|s| !s.script_name.starts_with("leak_")),
    ...
    fn gitignored_subrepo_rerooting_surfaces_nested_packages_but_honors_its_own_gitignore() {
        ...
        assert!(
            result
                .scripts
                .iter()
                .all(|s| s.script_name != "leak_private"),
    ...
    fn gitignored_git_repo_below_the_top_level_stays_excluded() {
        ...
        assert!(
            result
                .scripts
                .iter()
                .all(|s| s.script_name != "leak_deep_clone"),
    ...
    fn npm_workspaces_manifest_bounds_discovery_to_declared_globs() {
        ...
        assert!(
            find(&result.scripts, "build").is_some(),
            "packages/ui build found"
        );
        assert!(
            result
                .scripts
                .iter()
                .all(|s| s.script_name != "leak_example"),
    ...
    fn pnpm_workspace_yaml_bounds_discovery() {
        ...
        let result = discover_scripts(&tree.path());
        assert!(
            find(&result.scripts, "release").is_some(),
            "root release found"
        );
        assert!(find(&result.scripts, "dev").is_some(), "apps/web dev found");
        assert!(
        ...
        assert!(
            result
                .scripts
                .iter()
                .all(|s| s.script_name != "leak_scratch"),
    ...
    fn unbounded_walk_respects_max_scan_depth() {
        ...
        assert!(
            find(&scripts, "shallow_ok").is_some(),
            "a within-bound manifest surfaces"
        );
    }
    ...
    fn discovery_reports_zero_manifests_when_none_exist() {
        ...
        let result = discover_scripts(&tree.path());
        assert_eq!(
            result.manifests_found, 0,
            "no package.json => manifests_found 0"
        );
