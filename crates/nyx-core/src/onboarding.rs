...
pub fn merge_nyx_server(root: &mut Value, url: &str) -> OnboardConfigChange {
    ...
    let obj = root
        .as_object_mut()
        .expect("read_config guarantees an object root");
    let servers = obj
        .entry("mcpServers")
...
pub type PluginReconcileEntry = Option<(
    crate::plugin::PluginInstall,
    Box<dyn crate::plugin::PluginCli>,
)>;
...
pub fn reconcile_installed_plugins(plugins: &[(&str, PluginReconcileEntry)]) {
    for (key, maybe) in plugins {
        let Some((install, cli)) = maybe else {
            eprintln!(
                "nyx: skipping plugin reconcile for '{key}' (plugin path / CLI not resolvable)"
            );
            continue;
        };
        match crate::plugin::reconcile_with(install, cli.as_ref()) {
            Ok(crate::plugin::ReconcileOutcome::SkippedAbsent) => {
                eprintln!(
                    "nyx: plugin reconcile skipped for {key} (plugin absent; not re-installing)"
                );
            }
            ...
            Ok(crate::plugin::ReconcileOutcome::Updated) => {
                eprintln!(
                    "nyx: refreshed {key} plugin (re-copied + re-registered at the stable path)"
                );
            }
...
pub fn reconcile_providers_with_targets(port: u16, targets: &[(&str, Option<OnboardingTarget>)]) {
    let url = mcp_url(port);
...
mod tests {
    ...
    fn merge_updates_url_in_place_on_port_change() {
        ...
        assert_eq!(
            change,
            OnboardConfigChange::Updated {
                previous_url: "http://127.0.0.1:8765/mcp".into()
            }
        );
        // Updated IN PLACE: still exactly one entry, now on the new port.
        assert_eq!(root["mcpServers"].as_object().unwrap().len(), 1);
        assert_eq!(
            root["mcpServers"]["nyx"]["url"],
    ...
    fn onboard_preserves_unrelated_config_on_disk() {
        ...
        .unwrap();
        OnboardingTarget::new("Claude Code", &path)
            .onboard(8765)
            .unwrap();
        let v = read_json(&path);
    ...
    fn reconcile_present_entry_updates_url_in_place() {
        ...
        let targets: Vec<(&str, Option<OnboardingTarget>)> = vec![(
            "claude_code",
            Some(OnboardingTarget::new("Claude Code", &config_path)),
        )];
    ...
    fn reconcile_absent_entry_is_noop() {
        ...
        let v = read_json(&config_path);
        assert!(
            v["mcpServers"].get("nyx").is_none(),
            "nyx must not be added silently"
        );
    ...
    fn mcp_absent_when_nyx_server_entry_missing() {
        ...
        std::fs::write(
            &path,
            r#"{"mcpServers":{"other":{"type":"http","url":"http://x/mcp"}}}"#,
        )
        .unwrap();
        assert!(!mcp_server_present(&path));
    ...
    fn remove_legacy_mcp_server_strips_only_nyx() {
        ...
        .unwrap();
        assert!(
            remove_legacy_mcp_server(&path).unwrap(),
            "nyx present → stripped"
        );
        ...
        assert!(
            !remove_legacy_mcp_server(&missing).unwrap(),
            "missing config → nothing to strip"
        );
    }
    ...
    fn claude_mcp_installed_honors_the_config_seam() {
        ...
        std::fs::write(
            &path,
            r#"{"mcpServers":{"nyx":{"type":"http","url":"http://127.0.0.1:8765/mcp"}}}"#,
        )
        .unwrap();
        assert!(
            claude_mcp_installed(),
            "mcpServers.nyx present → installed via the seam"
        );
        ...
        assert!(
            !claude_mcp_installed(),
            "mcpServers.nyx gone → not installed via the seam"
        );
    ...
    impl crate::plugin::PluginCli for FakeReconcileCli {
        ...
        fn marketplace_list(
            ...
            Ok(self
                ...
                .clone()
                .map(|p| {
                    vec![crate::plugin::MarketplaceEntry {
                        name: "nyx".to_string(),
                        path: Some(p),
                    }]
                })
        ...
        *cli.registered.borrow_mut() = Some(base.join("DEAD-stale-path"));
        ...
        assert!(
            stable
                ...
                .exists(),
            "content re-copied to the stable dir on heal"
        );
    }
}
