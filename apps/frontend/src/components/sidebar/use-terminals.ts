 ...
 */
const TERMINALS_CHANGED_EVENT = "terminals://changed" as const;
...
async function resolveDefaultCwd(): Promise<string> {
  try {
    const home = await nyxBridge.paths.homeDir();
    return home || ".";
  } catch {
    return ".";
  }
...
export function useTerminals(): UseTerminals {
  ...
  const create = useCallback(async (cwd?: string) => {
    ...
    const row = await nyxBridge.invoke<TerminalRecord>("create_terminal", {
      cwd: resolved,
      label: null,
    });
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const rows = await nyxBridge.invoke<TerminalRecord[]>("list_terminals");
  ...
  useEffect(() => {
    if (!activeId) return;
    void nyxBridge.invoke("set_active", { id: activeId }).catch(() => {});
  }, [activeId]);
  ...
  const close = useCallback(async (id: string) => {
    ...
    await nyxBridge.invoke("close_terminal", { id }).catch(() => {});
  }, []);
  ...
  const markRead = useCallback((id: string) => {
    ...
    void nyxBridge.invoke("terminal_exec_mark_read", { id }).catch(() => {});
  }, []);
