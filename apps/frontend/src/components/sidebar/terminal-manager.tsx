...
export function TerminalManager() {
  ...
  const selectCommand = useCallback(
    (id: string) => {
      ...
      if (inst && inst.unread && (inst.state === "success" || inst.state === "error")) {
        void nyxBridge.invoke("command_acknowledge", { instanceId: id }).catch(() => {});
      }
    },
    [instances],
  );
  ...
  useEffect(() => {
    if (viewedCommandId) {
      void nyxBridge.invoke("command_acknowledge", { instanceId: viewedCommandId }).catch(() => {});
    }
  }, [viewedCommandId]);
  ...
  const handlePtyId = useCallback((recordId: string, ptyId: number | null) => {
    ...
    void nyxBridge.invoke("register_terminal_pty", { recordId, ptyId }).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    win.__nyx = {
      ...
      createCommand: (projectId, name, command, subfolder) =>
        nyxBridge.invoke<ManagedCommand>("command_create", {
          projectId,
          name,
          command,
      ...
      listCommandInstances: (workspaceId) =>
        nyxBridge.invoke<InstanceWithTemplate[]>("command_instance_list", { workspaceId }),
      commandStart: (instanceId) => nyxBridge.invoke<string>("command_start", { instanceId }),
      commandStop: (instanceId) => nyxBridge.invoke<string>("command_stop", { instanceId }),
      commandRelaunch: (instanceId) => nyxBridge.invoke<string>("command_relaunch", { instanceId }),
      commandState: (instanceId) => commandStates.get(instanceId) ?? "idle",
      commandOutput: (instanceId) => nyxBridge.invoke<string>("command_output", { instanceId }),
    };
    return () => {
