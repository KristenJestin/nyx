...
import { nyxBridge, type Unsubscribe } from "@/bridge";
...
async function start(
  ...
  // the callback (not on arrival) means the host only counts a chunk as "consumed"
  // once xterm has actually parsed it, so the per-terminal backlog reflects real
  // renderer progress. On shells without flow control (Tauri/fake) `ackPtyOutput` is
  // a no-op, so this stays shell-agnostic.
  function writeAndAck(payload: PtyOutputPayload): void {
  ...
  session.unlistenExit = await nyxBridge.subscribe<PtyExitPayload>("pty://exit", (payload) => {
  ...
  const id = await nyxBridge.invoke<number>("pty_spawn", { cwd, cols, rows, terminalId: recordId });

  if (session.torndown) {
    // Torn down before the spawn resolved: close the orphan, don't leak.
    void nyxBridge.invoke("pty_close", { id }).catch(() => {});
    return;
  }
  session.id = id;
  ...
  if (session.pendingOutput.length > 0) {
    ...
    for (const payload of buffered) {
      if (payload.id === id) writeAndAck(payload);
    }
  }
  ...
  session.disposables.push(
    term.onResize(({ cols: c, rows: r }) => {
      if (session.id === null) return;
      void nyxBridge.invoke("pty_resize", { id: session.id, cols: c, rows: r }).catch(() => {});
    }),
  );
...
function resyncSession(session: PtySession): void {
  ...
  void nyxBridge.invoke("pty_resize", { id: session.id, cols, rows }).catch(() => {});
}
